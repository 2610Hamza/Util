// lib/text.js
const STOP = new Set([
  'le','la','les','de','des','du','un','une','et','ou','pour','avec','a','à','au','aux','en','sur','dans','par','chez','mon',
  'ton','son','nos','vos','leurs','je','tu','il','elle','on','nous','vous','ils','elles','est','suis','faire','fait',
]);

const SYN = {
  plomberie: ['plombier','fuite','canalisation','robinet','wc','salle de bain'],
  electricite: ['électricien','lumière','prise','compteur','tableau','panne'],
  menage: ['ménage','nettoyage','femme de ménage','propreté'],
  informatique: ['pc','ordinateur','mac','panne','wifi','internet','réseau','dépannage'],
  coiffure: ['coiffeur','coiffure','coupe','brushing','coloration','barbier'],
  coaching: ['coach','accompagnement','mentorat','conseil','formation'],
  sante: ['médecin','docteur','consultation','infirmier','santé','psychologue'],
...
export function tokenize(str='') {
  ...
  for (const tok of base) {
    ...
    out.push(tok);
    // expansion synonyme si clé connue
    for (const [k, arr] of Object.entries(SYN)) {
      if (tok === k || arr.includes(tok)) {
        out.push(k, ...arr);
      }
    }
  }
